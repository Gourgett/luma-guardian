...
# 2. REAL DATA FETCHER
# ==========================================
DATA_FILE = "dashboard_state.json" # Fetches from main.py output
...
def get_scanner_data(data):
    ...
    df['Symbol'] = df['coin']
    df['Type'] = "PERP"
    df['Quality'] = df['quality'].apply(lambda x: "✅ Real Frenzy" if "BUY" in str(x) else ("❌ Whale Wick" if "SELL" in str(x) else "Waiting..."))
    df['Price'] = df['price']
    df['Vol (M)'] = df['vol_m']
...
def get_live_trades(data):
    ...
    df['Quality'] = df['PnL'].apply(lambda x: "✅ Winning" if x > 0 else "⚠️ Risk")
